...
|---|---|---|
| `WisedUpApplication` | `Application` | Initialize DataStore, create notification channel, install global crash handler. |
...
- Description: "Shown while WisedUp Focus is active so you always know your status."
- Created in `WisedUpApplication.onCreate()`.
...
4. **Auto Backup of DataStore** could clone `student.id` to a restored device. Configure `data_extraction_rules.xml` to exclude `wisedup_state.preferences_pb`.
