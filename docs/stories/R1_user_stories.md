...
- Persistence: Jetpack DataStore Preferences (file `wizedup_state.preferences_pb`)
...
**Acceptance criteria:**
- Given a clean checkout, when I run `./gradlew :android:assembleDebug`, then the build succeeds and produces an APK.
...
- Given the manifest, when inspected, then `<application>` references `WizedUpApplication`, `MainActivity` is the launcher, and the package layout matches `R1_architecture.md` §"Module / Package Structure".
- Given the manifest, when inspected, then the app declares `android:dataExtractionRules="@xml/data_extraction_rules"` and `android:fullBackupContent="@xml/backup_rules"` and both XML files exclude `wizedup_state.preferences_pb`.
- Given a release variant, when built, then R8 is enabled and the build succeeds.

**Test notes:**
- `./gradlew :android:assembleDebug :android:assembleRelease` both succeed.
...
- Given the onboarding screen, when rendered, then I see (a) a title "Welcome to WizedUp", (b) a single text field labeled "Your name", (c) a primary button "Continue" that is **disabled** while the trimmed input is empty.
...
**Acceptance criteria:**
- Given `FocusAccessibilityService` is **not enabled** in system settings, when I tap "Activate Focus", then the app shows a rationale dialog: title "One-time setup", body explaining "WizedUp uses Android Accessibility to keep you on the focus screen. Tap Open Settings, find WizedUp, and turn it on. Then come back.", primary button "Open Settings", secondary button "Cancel".
...
**Test notes:**
- Manual (emulator): fresh install → onboard → tap Activate → rationale dialog appears → Open Settings → toggle WizedUp on → press back → tap Activate again → notification permission prompt → Allow → focus screen appears.
...
- Given the notification, when posted, then channel ID = `focus_mode` (created in `WizedUpApplication.onCreate` with `IMPORTANCE_HIGH`), title = "Focus Mode active", text = "Tap to return to focus screen", `setOngoing(true)`, content intent = `PendingIntent` to `FocusActivity` with `FLAG_IMMUTABLE`.
...
- Manual: with focus active, run `adb shell am kill com.wizedup.focus` — service should restart (system-driven), notification reappears within ~5 s.
...
- Given `focus.is_active` = `true` and the foreground package becomes anything other than `com.wizedup.focus`, when `onAccessibilityEvent` fires with `TYPE_WINDOW_STATE_CHANGED`, then the service starts `FocusActivity` with `FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TOP | FLAG_ACTIVITY_REORDER_TO_FRONT` within 200 ms (measured on Pixel 6 emulator).
- Given `focus.is_active` = `false`, when `onAccessibilityEvent` fires, then the service does **nothing** (no relaunch, no log spam).
- Given the foreground package equals `com.wizedup.focus`, when the event fires, then no relaunch happens (avoid relaunch loops on our own activity transitions).
...
**Test notes:**
- Manual (API 34 emulator): clean install → activate → permission prompt → Allow. Then Settings → WizedUp → Notifications → Off → return to focus screen → app does not crash.
...
- Manual scenario matrix (run on Pixel 6 emulator API 34):
  ...
  | `adb shell am kill com.wizedup.focus` | Notification returns ≤ 5 s; activity returns on next foreground change |
...
- `./gradlew :android:assembleRelease :android:test` is green.
