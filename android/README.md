# WisedUp Focus — Android Brick (R1)
...
android/
  ...
  app/
    build.gradle.kts          // applicationId com.wisedup.focus
    proguard-rules.pro
    src/main/
      AndroidManifest.xml
      java/com/wisedup/focus/
        WisedUpApplication.kt
        MainActivity.kt
        ui/
...
4. In Settings → Accessibility → Installed Services → **WisedUp Focus**, toggle
...
WisedUp Focus uses the `AccessibilityService` API to detect when a non-focus app is
...
If a student goes to Settings → Apps → WisedUp Focus → **Force stop**, the foreground
...
For R1 we wire dependencies manually: `WisedUpApplication` holds two `lateinit`
repositories and any non-Composable component (services, receivers) calls
`WisedUpApplication.get()`. Hilt is a possible R2 refactor when the network/sync layer
