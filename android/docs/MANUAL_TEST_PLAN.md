...
> **Reset between sections:** Settings → Apps → WisedUp Focus → Storage → Clear data, then
...
### Steps (first launch)
1. Tap the WisedUp Focus icon.
2. Onboarding screen appears: title "Welcome to WisedUp", a "Your name" field, "Continue"
   disabled.
...
7. Find **WisedUp Focus**, toggle on, confirm the system warning.
...
| 8 | `adb shell am kill com.wisedup.focus.debug` | Notification reappears within ~5 s (START_STICKY). FocusActivity returns on next foreground change. |
...
4. Home: tap **Activate Focus** → Open Settings → toggle WisedUp Focus on → Back.
...
2. Trigger a backup: `adb shell bmgr backupnow com.wisedup.focus.debug`.
3. Uninstall, then reinstall via `adb install -r app-debug.apk` and trigger a restore:
   `adb shell bmgr restore com.wisedup.focus.debug`.
