...
DataStore file: `wizedup_state.preferences_pb`
...
### Neutral
- File location: `applicationContext.filesDir/datastore/wizedup_state.preferences_pb`. Backed up by Android Auto Backup unless we exclude it. Recommendation: exclude via `<data-extraction-rules>` so a restored device starts fresh; otherwise `student.id` could be cloned across devices. PM should flag this for the engineer.
...
R2 will:
1. Add Room database `WizedUp.db` v1 with `focus_sessions`, `bypass_events`, `pending_sync` tables.
